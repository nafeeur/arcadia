...
//!
//! A failed job could originally only be rerun through the object it belongs to: a document can be
//! re-extracted, a source can be re-synced; `bootstrap_ontology` and `adjudicate_entities` have no
//! such object to click. When balance runs out (#201 makes it fail on the first try), a whole batch
//! of documents stops at once, and after topping up you'd have to click them one by one. This gives
//! two entry points: within a KB (Editor) and globally (admin), scopable by kind and failure time —
//! the "retry" button on an alert passes exactly that incident's time window.
...
pub struct RequeueBody {
    ...
    /// Only requeue jobs that failed after this instant
    #[serde(default)]
...
/// Global requeue: system-level alerts (not tied to a KB) go through here. Admin-only — it touches jobs across all KBs
